...
const DeleteIcon = ({ width = 26, height = 26, color = "white" }) => {
  return (
...
const Editable = ({ children, onSave, onRemove }: componentProps) => {
  const thisElementRef = useRef<null | HTMLElement>(null);

  const copiedChild = cloneElement(Children.only(children), {
    ...
    <div className={`${styles.editable}`}>
      ...
      {onRemove && (
        <div className={`${styles.delete}`} onClick={(event) => onRemove(event)}>
          <DeleteIcon width={22} height={22}></DeleteIcon>
        </div>
      )}
    </div>
  );
